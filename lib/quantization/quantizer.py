...
def quant_int(x, scale, zero, maxq, eps=1e-9):
    q = torch.clamp(torch.round(x / scale.clamp_min(eps) + zero), 0, maxq)
    return q

def dequant_int(x, scale, zero, eps=1e-9):
    return scale * (x - zero)
...
class Quantizer(nn.Module):
    ...
    def configure(
        ...
        trits=False,
        round_zero=False, qq_scale_bits=None, qq_zero_bits=None,
        qq_groupsize=16, qq_zero_sym=False, reserved_bins: int=0,
        qqq_params=None,
    ):
        self.bits = bits
    ...
    def quant_int(self, x):
        if self.ready():
            return quant_int(x, self.scale, self.zero, self.maxq)

    def dequant_int(self, x):
        if self.ready():
            return dequant_int(x, self.scale, self.zero)
        return x
